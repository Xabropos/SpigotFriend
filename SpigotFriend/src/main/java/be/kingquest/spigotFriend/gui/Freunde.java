...
public class Freunde {
    ...
    public static void FreundeGUI(Player player, Set<String> online, Set<String> offline) {

        Inventory inventoryFreunde = Bukkit.createInventory(null, 54, "Freunde");
        ...
        if(alleFreundeZahl == 0){
            inventoryFreunde = inv("Du hast leider keine Freunde", inventoryFreunde);

            ItemStack KeineFreunde = new ItemStack(Material.PLAYER_HEAD);
            ItemMeta KeineFreundeMeta = KeineFreunde.getItemMeta();
            KeineFreundeMeta.setDisplayName("§4Du hast keine Freunde :(");
            KeineFreunde.setItemMeta(KeineFreundeMeta);

            inventoryFreunde.setItem(22, KeineFreunde);

        }else if(alleFreundeZahl<25){
            inventoryFreunde = inv("Deine Freunde", inventoryFreunde);
            for (String element : online) {
                ...
                inventoryFreunde.addItem(Player);
            }
            ...
            for (int page = 0; page < totalPages; page++) {
                inventoryFreunde = inv("Deine Freunde Seite " + (page + 1), inventoryFreunde);
                setNavigationItems(inventoryFreunde, page + 1, totalPages);
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    inventoryFreunde.addItem(friendItem);
                }
            }
        }


        player.openInventory(inventoryFreunde);
    }
    ...
    public static Inventory inv(String seite, Inventory inventory){
        ...
        inventory.setItem(1, HintergrundGrun);
        ...
        inventory.setItem(19, Hintergrund);
